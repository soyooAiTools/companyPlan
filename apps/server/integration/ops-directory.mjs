...
function normalizeOpsDirectory({ config, users, tenants, projects, tags, userDetails, projectMemberData }) {
  ...
  for (const item of projectMemberData) {
    ...
    for (const member of item.data?.members ?? []) {
      ...
      // 项目-成员-标签 映射(供 ops 按 项目+环节 找负责人)
      for (const tag of member.tags ?? []) {
        if (tag?.id != null && tag.id !== "") {
          projectMemberTags.push({
            projectId: toProjectId(item.projectId),
            personId: toUserId(member.user_id),
            tagId: `ops-tag-${tag.id}`,
          });
        }
      }
    }
  ...
  const people = Array.from(userRowsById.values()).map((user) => {
    ...
    return {
      ...
      wechatName: cleanText(user.wechat_name, 191),
      wechatAvatar: cleanText(user.wechat_avatar_url, 1024),
  ...
  const normalizedProjects = Array.from(projectRowsById.values())
    .map((project) => {
      ...
      return {
        ...
        tenantId: project.tenant_id != null && project.tenant_id !== "" ? `ops-tenant-${project.tenant_id}` : "",
        plannerName: cleanText(project.planner_name, 255),
        developerName: cleanText(project.developer_name, 255),
        genre: "试玩广告",
        channel: "Ops",
        ownerId,
        status,
