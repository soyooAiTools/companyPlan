      ...
      res.json(
        await pool.listMyProjectPool({
          ...
          advancedFilter: String(req.query.advanced_filter ?? ""),
				...
				statusChangedFrom: String(req.query.status_changed_from ?? ""),
				statusChangedTo: String(req.query.status_changed_to ?? ""),
    ...
    try {
      res.json(
        await pool.listArchivedProjectPool({
          ...
          endedFrom: String(req.query.ended_from ?? ""),
          endedTo: String(req.query.ended_to ?? ""),
          settledFrom: String(req.query.settled_from ?? ""),
          settledTo: String(req.query.settled_to ?? ""),
        }),
      );
    } catch (e) {
      soyooErrorResponse(res, e);
