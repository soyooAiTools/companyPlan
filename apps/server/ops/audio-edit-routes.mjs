import { listAudioEditSessions, updateAudioEditPriority, updateAudioEditRemark, updateAudioEditStatus } from "./services/ops-audio-edit.mjs";
...
export function registerAudioEditRoutes(app, { requireAuth }) {
  ...
  app.patch("/api/ops/audio-edit/sessions/:id/status", requireAuth, async (req, res) => {
    try {
      const data = await updateAudioEditStatus(req.params.id, req.body?.status, req.body?.remark);
      res.json(data);
    } catch (e) {
      if (e?.status === 400) {
        res.status(400).json({ error: e.message });
