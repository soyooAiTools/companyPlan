...
5. Validate changes with `npm run build` and, for UI changes, browser checks against admin and non-admin accounts.

## Product Guardrails

- Do not connect this project to Blueprint or Blueprint tooling.
- Do not reintroduce heavy WPS features the user rejected, especially `行高` and `导出`.
...
- The table should feel like a lightweight KDocs/WPS cloud table: title bar, collaborator/share area, toolbar, filters, status groups, grid body, statusbar, and sheet tabs.
...
- Admin sees global data and admin-only sheet tabs.
- Non-admin sees only `需求提单`, only relevant rows, and no admin navigation/tabs.
- Toolbar does not contain `行高` or `导出`.
