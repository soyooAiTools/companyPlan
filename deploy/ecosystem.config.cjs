...
// PM2 部署 —— OPS 后端(Node / Express,端口 4174)
//
// 业务配置(MySQL / OSS / soyoo 地址 / CORS 白名单 / cookie)统一放在 OPS .env.prod,
  ...
  apps: [
    {
      ...
      // 后端目录(相对本文件:deploy/.. → OPS,再进 apps/server),换机器无需改
      ...
      time: true,
      // 只设 APP_ENV → 触发加载 OPS .env.prod;密钥不进 PM2,避免被 pm2 dump 暴露
      env: {
        APP_ENV: "prod",
      },
    },
