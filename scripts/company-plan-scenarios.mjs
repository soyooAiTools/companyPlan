...
async function createRealisticTicket(page) {
  const title = `真实场景 UI 提研发联调 ${Date.now()}`;
  const tempDir = mkdtempSync(join(tmpdir(), "company-plan-ticket-"));
  const imagePath = join(tempDir, "cta-note.png");
  const attachmentPath = join(tempDir, "interaction-brief.md");
  ...
  await ticketFormField(page, "表格项目名称").locator("input").fill("Neon Chef 真实联调 - ui");
  await ticketFormField(page, "环节").locator("select").selectOption("研发");
  await ticketFormField(page, "负责人").locator("select").selectOption("u-dev");
  await ticketFormField(page, "优先级").locator("select").selectOption("P0");
  await ticketFormField(page, "任务类别").locator("input").fill("研发联调");
  await ticketFormField(page, "期望天数").locator("input").fill("2");
  await ticketFormField(page, "说明").locator("textarea").fill("UI 完成 CTA、失败页和引导标注后，提给研发联调点击热区与状态流转。");
  ...
  assert(relationText.includes("我提给 姜北"), "Creator should see the new ticket as 我提给 owner");
...
async function dragGanttBar(page, row, deltaX) {
  const bar = row.locator(".gantt-bar");
  const box = await bar.boundingBox();
  assert(Boolean(box), "Could not locate gantt bar box");
...
async function run() {
  ...
  try {
    ...
    const readonlyBefore = await devGanttRow.getAttribute("data-offset-days");
    ...
    await dragGanttBar(page, devGanttRow, 36);
    await page.waitForTimeout(150);
    assert((await devGanttRow.getAttribute("data-offset-days")) === readonlyBefore, "Programmer drag should not move gantt bars");
    ...
    const beforeOffset = Number(await adminGanttRow.getAttribute("data-offset-days"));
    const beforeStartAt = await adminGanttRow.getAttribute("data-start-at");

    await dragGanttBar(page, adminGanttRow, 42);
    await page.waitForFunction(
      ({ id, offset }) => {
        const row = document.querySelector(`.gantt-row[data-ticket-id="${id}"]`);
        return Number(row?.dataset.offsetDays) !== offset;
      },
      { id: ticketId, offset: beforeOffset }
    );

    const afterOffset = Number(await adminGanttRow.getAttribute("data-offset-days"));
    const adminGanttRowsAfter = await page.locator(".gantt-row").evaluateAll((rows) => rows.map((row) => row.dataset.ticketId));
    assert(afterOffset === Math.min(18, beforeOffset + 2), "Admin drag should move only the visual timeline offset by two days");
    ...
    assert(auditActions.includes("ticket_timeline_moved"), "Audit log should include admin gantt movement");

    await loginAs(page, "dev", "姜北");
    await clickSheetTab(page, "任务甘特图");
    const devGanttAfter = page.locator(`.gantt-row[data-ticket-id="${ticketId}"]`).first();
    await devGanttAfter.waitFor();
    assert(Number(await devGanttAfter.getAttribute("data-offset-days")) === afterOffset, "Programmer scoped gantt should reflect admin timeline offset");
